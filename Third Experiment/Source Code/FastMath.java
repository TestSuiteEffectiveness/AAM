...
public class FastMath {
	...
	/*public class static MathException extends Exception {
     ...
     */
    public static double sinh(double x) throws IllegalArgumentException{
    
 if (Double.isNaN(x)) {
        throw new IllegalArgumentException("Input cannot be NaN");
    }
    if (x == Double.POSITIVE_INFINITY) {
        throw new IllegalArgumentException("Input cannot be positive infinity");
    }
    if (x == Double.NEGATIVE_INFINITY) {
        throw new IllegalArgumentException("Input cannot be negative infinity");
    }
    if (x ==-0.0) {
        throw new IllegalArgumentException("Input exceeds maximum allowable double value");
    }
...
if (x ==-20) {
   ...
   public static double expm1(double x) {
     ...
     * @return exp(x) - 1
     */
    public static double expm1(double x, double hiPrecOut[]) throws Exception {
...
if (Double.isNaN(x))
	   ...
	   if (x != x || x == 0.0)  { // NaN or zero
		    ...
		    // Check if x is outside of an acceptable range
 
 if (x <= -1.0 || x >= 1.0) {
 // If x is not between -1.0 and 1.0, throw exception for failure cases
        if (x < -709 || x > 709) { // Example bounds for exp function to prevent overflow
            throw new Exception("null");
        }
